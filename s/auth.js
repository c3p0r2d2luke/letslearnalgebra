...
function messageMentionsUser(content, targetUsername) {
  ...
  if (mentionPattern.test(normalizedContent)) return true;
  return [...serverProfileByKey.entries()].some(([key, profile]) => {
    if (!key.startsWith(`${currentServerId}:`) || !profile?.display_name) return false;
    const discordUserId = key.slice(`${currentServerId}:`.length);
    return new RegExp(`<@!?${escapeRegExp(discordUserId)}>`, "i").test(normalizedContent)
      && profile.display_name.toLowerCase() === String(targetUsername).toLowerCase();
  });
}
...
function getChannelCheckpointKey() {
  return "lla_channel_read_checkpoints";
}

function readChannelCheckpointMap() {
  try {
    return JSON.parse(localStorage.getItem(getChannelCheckpointKey()) || "{}");
  } catch {
    return {};
  }
}

function writeChannelCheckpointMap(map) {
  localStorage.setItem(getChannelCheckpointKey(), JSON.stringify(map));
}

function setChannelCheckpoint(channelId, messageId = null) {
  if (!channelId) return;
  const map = readChannelCheckpointMap();
  map[String(channelId)] = {
    lastViewed: new Date().toISOString(),
    lastMessageId: messageId
  };
  writeChannelCheckpointMap(map);
}
...
async function markServerMessagesRead(serverId) {
  ...
  channelIds.forEach((channelId) => setChannelCheckpoint(channelId, lastMessageId));
...
function getChannelUnreadCount(channelId) {
  return unreadChannelCounts.get(Number(channelId)) || 0;
}

function getChannelMentionCount(channelId) {
  return channelMentionCounts.get(Number(channelId)) || 0;
}
...
async function refreshUnreadMentionCounts() {
  ...
  await Promise.all(serverIds.map(async (serverId) => {
    ...
    const channelReadMap = readChannelCheckpointMap();

    let query = supabaseClient
      .from("messages")
      .select("id, username, content, channel_id, inserted_at")
      .in("channel_id", channelIds)
    ...
    const unreadMessages = (data || []).filter((msg) => {
      ...
      const channelCheckpoint = channelReadMap[String(msg.channel_id)] || checkpoint;
      if (channelCheckpoint?.lastMessageId && Number(msg.id) <= Number(channelCheckpoint.lastMessageId)) return false;
      if (!channelCheckpoint?.lastMessageId && channelCheckpoint?.lastViewed && msg.inserted_at <= channelCheckpoint.lastViewed) return false;
      return true;
    });
    const count = unreadMessages.filter(msg => messageMentionsUser(msg.content, username)).length;
    channelIds.forEach((channelId) => {
      const channelMessages = unreadMessages.filter(msg => Number(msg.channel_id) === Number(channelId));
      unreadChannelCounts.set(Number(channelId), channelMessages.length);
      channelMentionCounts.set(Number(channelId), channelMessages.filter(msg => messageMentionsUser(msg.content, username)).length);
    });
    unreadMentionCounts.set(serverId, count);
  }));

  renderServerList();
  if (typeof renderChannelList === "function" && currentConversationType === "channel") renderChannelList();
}
  ...
  globalMentionSubscription = supabaseClient
    ...
    .on(
      ...
      async (payload) => {
        ...
        const isCurrentChannel = serverId === currentServerId
          && Number(message.channel_id) === Number(currentChannelId);
        if (isCurrentChannel) {
        ...
        if (messageMentionsUser(message.content, username)) {
        ...
        renderServerList();
        if (typeof renderChannelList === "function" && serverId === currentServerId) renderChannelList();
        if (messageMentionsUser(message.content, username)) {
          await notifyMentionClientSide(message, serverId);
        }
      }
