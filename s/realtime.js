  ...
  catNames.forEach(catName => {
    ...
    catChannels
      ...
      .forEach(ch => {
        ...
        const unreadCount = typeof getChannelUnreadCount === "function" ? getChannelUnreadCount(ch.id) : 0;
        const mentionCount = typeof getChannelMentionCount === "function" ? getChannelMentionCount(ch.id) : 0;
        if (unreadCount > 0) div.classList.add("channel-unread");
        if (mentionCount > 0) {
          const badge = document.createElement("span");
          badge.className = "channel-mention-badge";
          badge.textContent = mentionCount > 99 ? "99+" : String(mentionCount);
          badge.title = `${mentionCount} unread mention${mentionCount === 1 ? "" : "s"}`;
