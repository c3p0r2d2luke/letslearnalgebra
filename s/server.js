...
async function switchServer(serverId, updateUrl = true) {
  ...
  if (currentServerId && currentChannelId && currentServerId !== serverId) {
    localStorage.setItem(`lla_last_channel_${currentServerId}`, String(currentChannelId));
  }
  ...
  if (typeof syncDiscordStructure === "function") {
    await syncDiscordStructure(serverId);
  }
  await Promise.all([
  ...
  if (channels.length > 0) {
    console.log("📝 Loading last channel...");
  ...
  try {
    ...
    serverMembers = (members || []).map(m => {
      ...
      setServerProfileData(currentServerId, m.username, {
        ...
        is_bot: String(m.role || "").toLowerCase() === "bot"
      });
      return {
        ...m,
        role: effectiveRoleName,
...
function renderMemberList() {
  ...
  const renderGroup = (label, members) => {
    ...
    members.forEach(m => {
      ...
      const isBot = roleStr === "bot" || Boolean(getServerProfileData(currentServerId, m.username)?.is_bot);
      const avatarHtml = buildAvatarElement(m.username, "member-avatar").outerHTML;

      item.innerHTML = `
        ${avatarHtml}
        <div class="member-info">
          <div class="member-name" ${m.role_color ? `style="color:${escapeHTML(m.role_color)};"` : ""}>${escapeHTML(displayName(m.username))}${isBot ? ' <span class="bot-tag">Bot</span>' : ""}</div>
...
function subscribeToPresence() {
  ...
  memberRealtimeSubscription = supabaseClient
    ...
    .on("postgres_changes", {
      ...
      if (currentServerId === _memberRealtimeServerId) {
        await loadServerMembers();
      }
    })
    .subscribe();
...
async function openServerOptionsModal(serverId) {
  ...
  document.getElementById("serverOptionAfkChannel").value = discordSettings.afk_channel_id || "";
  document.getElementById("serverOptionSystemChannel").value = discordSettings.system_channel_id || "";
  document.getElementById("serverOptionRulesChannel").value = discordSettings.rules_channel_id || "";
  document.getElementById("serverOptionLocale").value = discordSettings.preferred_locale || "";
...
async function saveServerOptionsModal() {
  ...
  try {
    ...
    await syncDiscordGuildSetting(serverId, {
      ...
      afk_channel_id: document.getElementById("serverOptionAfkChannel").value.trim() || null,
      system_channel_id: document.getElementById("serverOptionSystemChannel").value.trim() || null,
      rules_channel_id: document.getElementById("serverOptionRulesChannel").value.trim() || null,
      preferred_locale: document.getElementById("serverOptionLocale").value.trim() || null
    });
