...
function updateMentionReadBar() {
  ...
  const text = document.getElementById("mentionReadBarText");
  if (!bar || !text || !currentChannelId || !username) return;
  const mentioned = [...messageDataMap.values()]
    .filter((message) => messageMentionsUser(message.content, username)
      && message.username !== username
      && (!currentChannelMentionReadThroughId || Number(message.id) > Number(currentChannelMentionReadThroughId)))
    .sort((a, b) => Number(a.id) - Number(b.id));
  currentChannelMentionMessageId = mentioned[0]?.id || null;
  if (!mentioned.length) {
    bar.hidden = true;
    return;
  }
  text.textContent = `${mentioned.length} mention${mentioned.length === 1 ? "" : "s"} in this channel`;
  bar.hidden = false;
}

function markMentionMessagesReadThrough(messageId = null) {
  if (!currentChannelId || !messageId) return;
  const mentioned = [...messageDataMap.values()]
    .filter((message) => messageMentionsUser(message.content, username)
      && Number(message.id) <= Number(messageId));
  if (!mentioned.length) return;
  currentChannelMentionReadThroughId = Number(messageId);
  setChannelCheckpoint(currentChannelId, Number(messageId));
  channelMentionCounts?.set(Number(currentChannelId), Math.max(
    0,
    (channelMentionCounts.get(Number(currentChannelId)) || 0) - mentioned.length
  ));
  updateMentionReadBar();
  if (typeof renderChannelList === "function") renderChannelList();
}

function handleMentionReadScroll() {
  if (!currentChannelMentionMessageId || !messagesList) return;
  const target = messagesMap.get(Number(currentChannelMentionMessageId));
  if (!target) return;
  const listRect = messagesList.getBoundingClientRect();
  const targetRect = target.getBoundingClientRect();
  if (targetRect.top < listRect.bottom && targetRect.bottom > listRect.top) {
    markMentionMessagesReadThrough(currentChannelMentionMessageId);
  }
}
...
async function switchChannelInternal(channelId) {
  ...
  currentChannelId = channelId;
  currentChannelMentionMessageId = null;
  currentChannelMentionReadThroughId = null;
  if (currentServerId) {
    localStorage.setItem(`lla_last_channel_${currentServerId}`, String(channelId));
  }
...
async function loadMessages() {
  ...
  if (discordChannel?.discord_server_id) {
    ...
    (discordProfiles || []).forEach((profile) => {
      ...
      setServerProfileData(currentServerId, profileUsername, {
        ...
        role: member.role || "",
        role_color: "",
        is_bot: String(member.role || "").toLowerCase() === "bot"
      });
    });
  ...
  const [
    ...
    messageIds.length > 0
      ? supabaseClient.from("discord_message_mapping")
  ...
  uniqueMessages.forEach((message) => {
    const mapping = discordMappingByMessageId.get(Number(message.id));
    if (mapping) Object.assign(message, {
      ...
      if (discordSync) {
        const discordResponse = await fetch(`${supabaseUrl}/functions/v1/discord-message-sync`, {
          ...
          body: JSON.stringify({
            ...
            content,
            username,
            reply_to_message_id: replyingTo || null,
...
function createMessageElement(msg) {
    ...
    try {
      ...
      if (discordEmbeds.length) {
        const duplicateLines = new Set(
          discordEmbeds.flatMap((embed) => [embed.url, embed.title])
            .filter(Boolean)
            .map((value) => String(value).replace(/[\u200B-\u200D\uFEFF]/g, "").trim())
        );
        visibleContent = visibleContent
          .split("\n")
          .filter((line) => !duplicateLines.has(line.replace(/[\u200B-\u200D\uFEFF]/g, "").trim()))
          .join("\n")
          .trim();
      }
    } catch (error) {
  ...
  visibleContent = visibleContent.replace(/[\u200B-\u200D\uFEFF]/g, "");
  visibleContent = visibleContent.replace(/<@!?(\d+)>/g, (match, discordUserId) => {
    const profile = getServerProfileData(currentServerId, `discord-${discordUserId}`);
  ...
  header.innerHTML = `${escapeHTML(displayName(msg.username))}${serverProfile?.is_bot ? ' <span class="bot-tag">Bot</span>' : ""}<span class="msg-timestamp">${timestamp}</span>`;
  ...
  if (fileMatch) {
    ...
    formatted = formatted.replace(/__LLA_ROLE_MENTION_(\d+)__/g, '<span class="mention role-mention" data-role-id="$1">@role</span>');
    ...
    resolveDiscordRoleMentions(contentDiv, currentServerId);
    ...
    function createDiscordEmbedElement(embed) {
      ...
      if (embed.author?.name) {
        const author = embed.author.url ? document.createElement("a") : document.createElement("div");
        author.className = "discord-embed-author";
        if (embed.author.url) {
          author.href = getSafeUrl(embed.author.url) || "#";
      ...
      if (Array.isArray(embed.fields) && embed.fields.length) {
        ...
        embed.fields.forEach((field) => {
          ...
          if (field.inline) item.classList.add("is-inline");
      ...
      if (embed.footer?.text) {
        ...
        const footerIconUrl = getSafeUrl(embed.footer.icon_url || "");
        if (footerIconUrl) {
          const icon = document.createElement("img");
          icon.src = footerIconUrl;
          icon.alt = "";
          icon.loading = "lazy";
        ...
        if (embed.timestamp) {
          ...
          timestamp.textContent = new Date(embed.timestamp).toLocaleDateString([], { year: "numeric", month: "numeric", day: "numeric" });
...
async function handleRealtimeMessage(newMsg, eventType) {
  if (!newMsg) return;
  if (eventType === "INSERT" && newMsg.username !== username
    && String(newMsg.channel_id) !== String(currentChannelId)) {
    const channelId = Number(newMsg.channel_id);
    ...
    if (serverId) {
      ...
      if (messageMentionsUser(newMsg.content, username)) {
        channelMentionCounts.set(channelId, (channelMentionCounts.get(channelId) || 0) + 1);
...
async function addReaction(messageId, emoji) {
  ...
  try {
    ...
    if (existing && existing.length > 0) {
      ...
      const message = messageDataMap.get(normalizedMessageId);
      if (message?.discord_message_id && typeof supabaseUrl !== "undefined") {
        const discordResponse = await fetch(`${supabaseUrl}/functions/v1/discord-message-sync`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        if (!discordResponse.ok) console.warn("Discord reaction removal sync failed:", await discordResponse.text());
      }
      ...
      if (message?.discord_message_id && typeof supabaseUrl !== "undefined") {
        ...
        if (!discordResponse.ok) console.warn("Discord reaction sync failed:", await discordResponse.text());
      }
    }
...
const discordRoleNameCache = new Map();

async function resolveDiscordRoleMentions(container, serverId) {
  const mentions = [...container.querySelectorAll(".role-mention[data-role-id]")];
  if (!mentions.length || !serverId) return;
  let roleNames = discordRoleNameCache.get(serverId);
  if (!roleNames) {
    const { data } = await supabaseClient
      .from("discord_roles")
      .select("discord_role_id, discord_role_name")
      .eq("discord_server_id", serverId);
    roleNames = new Map((data || []).map((role) => [
      String(role.discord_role_id),
      role.discord_role_name || "role"
    ]));
    discordRoleNameCache.set(serverId, roleNames);
  }
  mentions.forEach((mention) => {
    const roleName = roleNames.get(mention.dataset.roleId);
    if (roleName) mention.querySelector(".mention-name")?.replaceChildren(document.createTextNode(roleName));
    if (roleName) mention.textContent = `@${roleName}`;
  });
}
...
function formatMessageContent(content, role) {
  ...
  // Discord role mentions arrive as escaped `<@&role-id>` tokens. Keep them
  // visibly distinct from plain text while preserving the Discord target.
  escaped = escaped.replace(/&lt;@&amp;(\d+)&gt;/g, "__LLA_ROLE_MENTION_$1__");
...
async function loadDefaultChannel() {
  ...
  const savedChannelId = currentServerId
    ? localStorage.getItem(`lla_last_channel_${currentServerId}`)
    : null;
  const savedChannel = savedChannelId
    ? channels.find((channel) => String(channel.id) === String(savedChannelId))
    : null;
  const generalChannel = channels.find(ch => String(ch.name || "").toLowerCase() === "general");
  const targetChannelId = savedChannel?.id || generalChannel?.id || channels[0].id;
