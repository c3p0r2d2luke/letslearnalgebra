...
(function setupGifSlashCommand() {
  ...
  function setState(html, kind = "") {
    picker.innerHTML = `
      ...
      GIF Search <span style="float:right; opacity:0.6">Powered by Klipy</span>
      </div>
  ...
  function renderResults(results, query) {
    ...
    header.innerHTML = `GIFs for "${query}" <span style="float:right; opacity:0.6">Powered by Klipy</span>`;
    picker.innerHTML = "";
    picker.appendChild(header);

    results.forEach((r) => {
      const m = r.media_formats || r.media || r.file || null;
      const hd = m?.hd || m;
      const md = m?.md || m?.medium || m;
      const thumb = m?.tinygif?.url || m?.nanogif?.url || m?.mediumgif?.url
        || md?.webp?.url || md?.jpg?.url || hd?.webp?.url
        || r.thumbnail_url || r.preview_url;
      const full = m?.gif?.url || hd?.gif?.url || md?.gif?.url
        || hd?.webp?.url || r.gif_url || r.url;
  ...
  async function fetchKlipy(query) {
    if (klipyCache.has(query)) return klipyCache.get(query);
    try {
      const results = await resolveGifEdge(query);
      const normalized = Array.isArray(results) ? results.slice(0, KLIPY_LIMIT) : [];
      klipyCache.set(query, normalized);
      return normalized;
    } catch (e) {
  ...
  open(inputEl.getBoundingClientRect());
    ...
    searchTimer = setTimeout(async () => {
      ...
      try {
        const results = await fetchKlipy(query);
        if (seq !== searchSeq) return;
