  ...
  guilds.forEach((guild) => {
    ...
    html += `
      <div style="display: flex; align-items: center; padding: 10px; border-bottom: 1px solid #333; cursor: pointer;" onclick="selectDiscordGuild('${guild.id}')">
        ...
        <input type="radio" name="discord-guild" value="${guild.id}" style="cursor: pointer;" onclick="event.stopPropagation()" />
      </div>
    `;
  });
  html += "</div>";
...
async function importSelectedDiscordGuilds() {
  ...
  if (selected.length !== 1) {
    alert("❌ Select exactly one Discord server to import.");
    return;
  }

  const shouldImport = window.confirm(
    "⚠️ Before importing, make sure the \"LLA Chat Bot\" bot is already in every selected Discord server. " +
    "If it is not there, the import will not work. Only one server can be imported at a time. Continue?"
  );
  ...
  try {
    ...
    const importedSummary = results.map((result) => {
      ...
      return `\n${imported.channels || 0} channels mapped, ${imported.members || 0} members imported, and ${imported.messages || 0} messages imported.${warnings}`;
    }).join("");
...
async function syncDiscordStructure(serverId = currentServerId) {
  ...
  try {
    const response = await fetch(`${supabaseUrl}/functions/v1/discord-message-sync`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "sync_structure", server_id: serverId }),
    });
    if (!response.ok) {
      console.warn("Failed to refresh Discord server structure:", await response.text());
    }
  } catch (error) {
    console.warn("Error refreshing Discord server structure:", error);
  }
}
