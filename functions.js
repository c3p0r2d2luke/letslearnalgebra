...
function updateMessageLock() {
  const hasName = !!(nameInput?.value && nameInput.value.trim().length > 0);
    ...
    .on(
      ...
      (payload) => {
        ...
        handleRealtimeMessage(
          record,
          eventType || payload.eventType || payload.type || payload.event,
        );
...
async function loadMessages() {
  try {
    const data = await supaFetch("messages", {
      query: { select: "*", order: "inserted_at.asc" },
    });
...
async function loadUser() {
  ...
  try {
    ...
    const users = await supaFetch("users", {
      query: { select: "role", username: `ilike.${stored}` },
    });
...
async function saveName() {
  ...
  try {
    ...
    const users = await supaFetch("users", {
      query: { select: "role", username: `eq.${entered}` },
    });
    const user = Array.isArray(users) ? users[0] : null;
    currentRole = user?.role || "User";
    localStorage.setItem("chatRole", currentRole);
...
async function sendMessage() {
  ...
  try {
    const users = await supaFetch("users", {
      query: { select: "blocked", username: `eq.${username}` },
    });
    const user = Array.isArray(users) ? users[0] : null;
    if (user?.blocked) {
  ...
  if (roleView === "Admin") {
    ...
    btnChangeName.onclick = async () => {
      const newName = prompt(
        "New username for " + message.username + ":",
        message.username,
      );
      if (!newName) return;
      try {
        ...
        await supaFetch("messages", {
          method: "PATCH",
          query: { username: `eq.${message.username}` },
          body: { username: newName },
        });
    ...
    btnExport.onclick = () => {
      const snapshot = Array.from(messagesList.querySelectorAll("li")).map(
        (liEl) => ({
    ...
    btnDeleteKeyword.onclick = async () => {
      ...
      if (!keyword) return;
      try {
        const allMessages = await supaFetch("messages", {
          query: { select: "*" },
        });
        const matches = (Array.isArray(allMessages) ? allMessages : [])
        ...
        await supaFetch("messages", {
          method: "DELETE",
          query: { id: `in.(${matches.join(",")})` },
        });
        // Remove from DOM
        document.querySelectorAll("#messages li").forEach((liEl) => {
          if (liEl.querySelector(".content")?.textContent.includes(keyword))
            liEl.remove();
        });
    ...
    btnNuke.onclick = async () => {
      if (!confirm("Delete " + message.username + " and all their messages?"))
        return;
      try {
        await supaFetch("messages", {
          method: "DELETE",
        ...
        document.querySelectorAll("#messages li").forEach((liEl) => {
          if (liEl.querySelector(".username")?.textContent === message.username)
            liEl.remove();
        });
    ...
    btnPromoteManager.onclick = async () => {
      try {
        await supaFetch("users", {
          method: "POST",
          query: { on_conflict: "username" },
          body: { username: message.username, role: "Manager" },
        });
    ...
    btnPromoteAdmin.onclick = async () => {
      try {
        await supaFetch("users", {
          method: "POST",
          query: { on_conflict: "username" },
          body: { username: message.username, role: "Admin" },
        });
    ...
    btnDemote.onclick = async () => {
      try {
        await supaFetch("users", {
          method: "POST",
          query: { on_conflict: "username" },
          body: { username: message.username, role: "User" },
        });
    ...
    btnPin.onclick = async () => {
      try {
        ...
        await supaFetch("messages", {
          method: "PATCH",
          query: { id: `eq.${message.id}` },
          body: { is_pinned: !currentlyPinned },
        });
        ...
        log(
          (!currentlyPinned ? "📌 Pinned" : "❌ Unpinned") +
            " message id=" +
            message.id,
        );
        if (!currentlyPinned)
  ...
  if (roleView === "Manager") {
    ...
    li.addEventListener("click", (e) => {
      ...
      managerControls.style.display =
        managerControls.style.display === "none" ? "flex" : "none";
    });
  ...
  if (roleView !== "User") {
    li.addEventListener("click", (e) => {
      ...
      adminControls.style.display =
        adminControls.style.display === "none" ? "flex" : "none";
    });
    document.addEventListener("click", () => {
      adminControls.style.display = "none";
    });
