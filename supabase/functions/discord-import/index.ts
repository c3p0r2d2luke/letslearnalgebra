...
async function importDiscordServer(
  ...
  try {
    ...
    if (botToken && Array.isArray(discordChannels)) {
      const messageChannels = discordChannels.filter((channel) => [0, 5].includes(channel.type));
      diagnostics.message_channels_attempted = messageChannels.length;
      const messageResults = await Promise.all(messageChannels.map(async (discordChannel) => {
        const messagesResponse = await fetch(
          `${DISCORD_API}/channels/${discordChannel.id}/messages?limit=100`,
          { headers: fetchHeaders }
        );
        if (!messagesResponse.ok) {
          return { channel: discordChannel, messages: [], error: `messages:${discordChannel.id} ${messagesResponse.status}: ${await messagesResponse.text()}` };
        }
        return { channel: discordChannel, messages: await messagesResponse.json(), error: null };
      }));
      for (const result of messageResults) {
        if (result.error) diagnostics.api_errors.push(result.error);
        else {
          discordMessagesByChannel.set(result.channel.id, result.messages);
    ...
    if (Array.isArray(discordChannels)) {
      for (const discordChannel of discordChannels) {
        ...
        const importedMessages = (discordMessagesByChannel.get(discordChannel.id) || []).reverse();
        const messageRows = [];
        const messageMappings = [];
        for (const discordMessage of importedMessages) {
          ...
          messageMappings.push({ discordMessageId: discordMessage.id });
        }
        if (messageRows.length) {
          const { data: nativeMessages, error: messageError } = await supabaseClient
            .from("messages")
            .insert(messageRows)
            .select("id");
          if (messageError || !nativeMessages) {
            diagnostics.message_inserts_failed += messageRows.length;
          } else {
            await supabaseClient.from("discord_message_mapping").insert(
              nativeMessages.map((nativeMessage, index) => ({
                discord_server_id: discordServer.id,
                discord_channel_id: discordChannel.id,
                chat_message_id: nativeMessage.id,
                discord_message_id: messageMappings[index].discordMessageId,
              }))
            );
          }
        }
